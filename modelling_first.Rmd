...
**1)** kBased on this chocolate model, what would the predicted value of winpercent be for candies that contain chocolate?
...
left_join(
  transfer_data %>% 
    mutate(Q6 = str_detect(Q6, "sugarpercent$") %>% as.integer()) %>% 
    select(id, Q6),
  ...
  select(-term, -id) %>% 
  rename(other_1 = Q19, other_2 = Q21, other_3 = Q40) %>% 
