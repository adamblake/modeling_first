...
se <- function(x, na.rm = TRUE) {
  sd(x, na.rm) / sqrt(length2(x, na.rm))
}

# margin of error
moe <- function(x, p = .975, na.rm = TRUE) {
  se(x, na.rm) * qt(p, length2(x, na.rm) - 1)
}
...
The data collected for this investigation were collected across two quarters (F18, W19) at the University of California, Los Angeles. Demographic information about the participants is reported in the table below.
...
knitr::include_graphics("figures/output_fit_chocolate.png")
knitr::include_graphics("figures/output_anova_chocolate.png")
```
...
```{r, echo = FALSE, out.width = '100%'}
knitr::include_graphics("figures/output_fit_sugar.png")
knitr::include_graphics("figures/output_anova_sugar.png")
```
...
```{r mc_analysis_function}
mc_analysis <- function(data, column) {
  data %>%
    select_at(column) %>% 
    na.omit() %>%
...
#### PRE
```{r}
transfer_data %>% mc_analysis("Q2")
```

#### SST
```{r}
transfer_data %>% mc_analysis("Q3")
```

#### SSE
```{r}
transfer_data %>% mc_analysis("Q4")
```
